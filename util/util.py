...
def tensor2vis(image_tensor, imtype=np.uint8, normalize=True):
...
#    print(np.min(image_numpy))
    max_x = np.max(image_numpy)
    min_x = np.min(image_numpy)
    
    image_numpy = 255/(max_x - min_x)*(image_numpy - min_x)
    print(np.max(image_numpy))
    print(np.min(image_numpy))
